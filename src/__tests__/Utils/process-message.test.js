...
const createBaseMessage = (key, message) => {
    return {
        key: Object.assign({ remoteJid: 'chat@s.whatsapp.net', fromMe: false, id: 'ABC' }, key),
...
describe('cleanMessage', () => {
    ...
    describe('Reaction Message Perspective', () => {
        it("should correct the perspective of a reaction to another user's message", () => {
            const message = createBaseMessage({ fromMe: false, participant: otherUserId }, {
                reactionMessage: {
                    key: {
        ...
        it('should not modify a reaction on a message I sent from another device', () => {
            const message = createBaseMessage({ fromMe: true }, {
                reactionMessage: {
                    key: { remoteJid: 'chat@s.whatsapp.net', fromMe: true, id: 'MSG_I_SENT' },
                    text: '❤️'
                }
            });
            const originalReactionKey = Object.assign({}, message.message.reactionMessage.key);
            cleanMessage(message, meId, meLid);
            const reactionKey = message.message.reactionMessage.key;
