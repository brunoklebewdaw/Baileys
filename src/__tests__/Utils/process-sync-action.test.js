...
const createSyncAction = (action, index = ['type', 'id', 'msgId', '0']) => ({
    syncAction: { value: action },
    index
});
const mockMe = { id: 'me@s.whatsapp.net', name: 'Test User' };
describe('processSyncAction', () => {
    ...
    describe('muteAction', () => {
        it('emits chats.update with muteEndTime when muted', () => {
            ...
            expect(ev.emit).toHaveBeenCalledWith('chats.update', expect.arrayContaining([expect.objectContaining({ id: 'chat123@s.whatsapp.net', muteEndTime: 1700000000 })]));
        });
        it('emits null muteEndTime when unmuted', () => {
            ...
            processSyncAction(syncAction, ev, mockMe, undefined, logger);
            expect(ev.emit).toHaveBeenCalledWith('chats.update', expect.arrayContaining([expect.objectContaining({ muteEndTime: null })]));
        });
    });
    describe('archiveChatAction', () => {
        it('emits chats.update with archived true/false', () => {
            ...
            expect(ev.emit).toHaveBeenCalledWith('chats.update', expect.arrayContaining([expect.objectContaining({ archived: true })]));
        });
    ...
    describe('markChatAsReadAction', () => {
        it('emits unreadCount 0 when read is true', () => {
            ...
            expect(ev.emit).toHaveBeenCalledWith('chats.update', expect.arrayContaining([expect.objectContaining({ unreadCount: 0 })]));
        });
        it('emits unreadCount -1 when read is false', () => {
            ...
            expect(ev.emit).toHaveBeenCalledWith('chats.update', expect.arrayContaining([expect.objectContaining({ unreadCount: -1 })]));
        });
        it('emits null unreadCount during initial sync when already read', () => {
            ...
            expect(ev.emit).toHaveBeenCalledWith('chats.update', expect.arrayContaining([expect.objectContaining({ unreadCount: null })]));
        });
    });
    ...
    describe('pushNameSetting', () => {
        it('emits creds.update when name differs', () => {
            ...
            processSyncAction(syncAction, ev, mockMe, undefined, logger);
            expect(ev.emit).toHaveBeenCalledWith('creds.update', { me: Object.assign(Object.assign({}, mockMe), { name: 'New' }) });
        });
    ...
    describe('pinAction', () => {
        it('emits chats.update with pinned timestamp or null', () => {
            ...
            processSyncAction(syncAction, ev, mockMe, undefined, logger);
            expect(ev.emit).toHaveBeenCalledWith('chats.update', expect.arrayContaining([expect.objectContaining({ pinned: 1700000000 })]));
        });
    });
    describe('starAction', () => {
    ...
    describe('labelEditAction', () => {
        it('emits labels.edit', () => {
            const syncAction = createSyncAction({ labelEditAction: { name: 'Important', color: 1, deleted: false, predefinedId: 5 } }, ['label', 'label123']);
            processSyncAction(syncAction, ev, mockMe, undefined, logger);
    ...
    describe('unprocessable actions', () => {
        it('logs debug for unknown action', () => {
            const syncAction = createSyncAction({ unknownAction: {} }, [
                'unknown',
                'id123'
            ]);
            processSyncAction(syncAction, ev, mockMe, undefined, logger);
