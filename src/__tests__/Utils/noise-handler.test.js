...
describe('Noise Handler', () => {
    describe('decodeFrame with multiple frames in buffer', () => {
        it('should process multiple unencrypted frames in single buffer', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const onFrame = (frame) => {
                receivedFrames.push(Buffer.from(frame));
            };
            yield handler.decodeFrame(combinedBuffer, onFrame);
            expect(receivedFrames).toHaveLength(2);
            expect(receivedFrames[0]).toEqual(payload1);
            expect(receivedFrames[1]).toEqual(payload2);
        }));
        it('should handle frames split across multiple decodeFrame calls', () => __awaiter(void 0, void 0, void 0, function* () {
            const keyPair = Curve.generateKeyPair();
            const logger = createMockLogger();
            const handler = makeNoiseHandler({
                keyPair,
            ...
            yield handler.decodeFrame(part1, onFrame);
            expect(receivedFrames).toHaveLength(0);
            yield handler.decodeFrame(part2, onFrame);
            expect(receivedFrames).toHaveLength(1);
            expect(receivedFrames[0]).toEqual(payload);
        }));
        it('should correctly process frames when callback triggers async operations', () => __awaiter(void 0, void 0, void 0, function* () {
            const keyPair = Curve.generateKeyPair();
            const logger = createMockLogger();
            const handler = makeNoiseHandler({
                keyPair,
    ...
    describe('encrypted frame handling', () => {
        it('should encrypt and verify frame structure', () => __awaiter(void 0, void 0, void 0, function* () {
            const keyPair = Curve.generateKeyPair();
            const logger = createMockLogger();
            const handler = makeNoiseHandler({
                keyPair,
        ...
        it('should produce different ciphertext for same plaintext due to counter', () => __awaiter(void 0, void 0, void 0, function* () {
            const keyPair = Curve.generateKeyPair();
            const logger = createMockLogger();
            const handler = makeNoiseHandler({
                keyPair,
    ...
    describe('race condition scenario - concurrent decodeFrame calls', () => {
        it('should handle concurrent decodeFrame calls without corrupting inBytes buffer', () => __awaiter(void 0, void 0, void 0, function* () {
            const keyPair = Curve.generateKeyPair();
            const logger = createMockLogger();
            const handler = makeNoiseHandler({
                keyPair,
            ...
            yield Promise.all(frames.map(frame => handler.decodeFrame(frame, onFrame)));
        ...
        it('should maintain counter integrity with many frames in single buffer', () => __awaiter(void 0, void 0, void 0, function* () {
            const keyPair = Curve.generateKeyPair();
            const logger = createMockLogger();
            const handler = makeNoiseHandler({
                keyPair,
    ...
    describe('encrypted frame race condition', () => {
        ...
        it('should serialize concurrent decodeFrame calls (fix for race condition)', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const receivedOrder = [];
            const onFrame = (frame) => {
                const content = Buffer.from(frame).toString();
        ...
        it('should maintain frame order with interleaved partial frames after fix', () => __awaiter(void 0, void 0, void 0, function* () {
