...
export const getDecryptionJid = (sender, repository) => __awaiter(void 0, void 0, void 0, function* () {
    if (isLidUser(sender) || isHostedLidUser(sender)) {
        return sender;
    }
    const mapped = yield repository.lidMapping.getLIDForPN(sender);
    return mapped || sender;
});
const storeMappingFromEnvelope = (stanza, sender, repository, decryptionJid, logger) => __awaiter(void 0, void 0, void 0, function* () {
    // TODO: Handle hosted IDs
    const { senderAlt } = extractAddressingContext(stanza);
    if (senderAlt && isLidUser(senderAlt) && isPnUser(sender) && decryptionJid === sender) {
        try {
            yield repository.lidMapping.storeLIDPNMappings([{ lid: senderAlt, pn: sender }]);
            yield repository.migrateSession(sender, senderAlt);
            ...
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (Array.isArray(stanza.content)) {
                    for (const { tag, attrs, content } of stanza.content) {
                        ...
                        decryptables += 1;
                        let msgBuffer;
                        const decryptionJid = yield getDecryptionJid(author, repository);
                        if (tag !== 'plaintext') {
                            // TODO: Handle hosted devices
                            yield storeMappingFromEnvelope(stanza, author, repository, decryptionJid, logger);
                        }
                        try {
