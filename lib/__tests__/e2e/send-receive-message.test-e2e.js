...
describe('E2E Tests', () => {
    ...
    test('should send and download an image message', async () => {
        ...
        try {
            ...
            const buffer = await downloadMediaMessage(receivedMsg, 'buffer', {}, {
                logger: sock.logger,
        ...
        try {
            ...
            const receivedVideoPromise = new Promise((resolve, reject) => {
                videoListener = ({ messages }) => {
                    const msg = messages.find(m => m.key.remoteJid === groupJid && m.message?.videoMessage?.caption === originalCaption);
                    if (msg)
                        resolve(msg);
                };
            ...
            const receivedCommandPromise = new Promise((resolve, reject) => {
                commandListener = ({ messages }) => {
                    const msg = messages.find(m => m.key.remoteJid === groupJid && m.message?.extendedTextMessage?.text === commandText);
                    if (msg)
                        resolve(msg);
                };
                sock.ev.on('messages.upsert', commandListener);
...
//# sourceMappingURL=send-receive-message.test-e2e.js.map